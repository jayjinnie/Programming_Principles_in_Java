public class Database {
	...
	public static void main(String[] args) {
		...
		boolean register1 = db.insert(c1); // BankAccount는 Record를 구현한 클래스이므로 Record 타입 수용 가능
		...
		Record r = db.find(k);
		System.out.println(((BankAccount)r).getBalance()); // Record r에는 getBalance 메소드가 없음, 따라서 BankAccount로 타입 캐스트가 필요
		// 타입 캐스팅 대상 오브젝트(r)가 실제로 BankAccount의 오브젝트인지 체크해야함! (instanceof 서브타입 여부 검사)
		// 서브타입 여부 검사는 컴파일러가 할 수 없기 때문에 주의해야 함
	}
}